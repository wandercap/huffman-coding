...
static void compacta_arquivo(char *source, size_t bufsize, char *output, unsigned long int *tabelaCodigos) {
    ...
    #pragma omp parallel shared(total) num_threads(NUMTHREADS)
    {
        size_t i, tam;
        char bit, c, x;
        ...
        char *buffer = malloc(sizeof(char) * (bufsize / NUMTHREADS));
        ...
        for (i = 0; i < NUMTHREADS; i++) {
            ...
            {
                size_t j, k = 0;
                out = (char *)realloc(out, sizeof(char)*(total+tam));
                for (j = total; j < tam + total; j++) {
                    out[j] = buffer[k];
                    k++;
                }
            total += tam;
...
static void descompacta_arquivo(char *source, size_t bufsize, char *output, node_p raiz) {
    ...
    #pragma omp parallel shared(total, mascara, raiz) num_threads(NUMTHREADS)
    {
        int c;
        size_t i, j;
        char bit;
