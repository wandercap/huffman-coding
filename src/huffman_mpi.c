...
static file_p ler_arquivo(char *input, int option, int numProcesses, int myRank) {
    size_t inputFileLength;
    FILE *in;
    file_p file;
    MPI_File mpi_inputFile;
    MPI_Status status;
    ...
    MPI_Bcast(&inputFileLength, 1, my_MPI_SIZE_T, 0, MPI_COMM_WORLD);
    file->bufsize = (inputFileLength / numProcesses);

    if (myRank == (numProcesses - 1)) {
        file->bufsize = inputFileLength - ((numProcesses-1) * file->bufsize);   
    }

    MPI_File_open(MPI_COMM_WORLD, input, MPI_MODE_RDONLY, MPI_INFO_NULL, &mpi_inputFile);
    MPI_File_seek(mpi_inputFile, myRank * file->bufsize, MPI_SEEK_SET);

    file->source = malloc(sizeof(char) * file->bufsize);
    MPI_File_read(mpi_inputFile, file->source, file->bufsize, MPI_CHAR, &status);

    if(option == 2) { // Essa parte ainda não está adaptada pro MPI. Será necessário guardar o tamanho de cada bloco de compactação,
                      // para poder obter o offset necessário para ler as N árvores
        int i;
        char buff[1024];
...
int main(int argc, char *argv[]) {
    ...
    if(strcmp(argv[1], "1") == 0) {
        ...
        file = ler_arquivo(input, 1, myRank);
